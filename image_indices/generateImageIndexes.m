...
for cont = 1 : size(listgnm,1)
  %GAMBIARRA pra garantir que não restem espaços. NÃO ALTERAR! trim não é[MS, nSegsMS, hist] = im2segs2(pathMS);
  %uma solução completamente confiável!
  true_value = num2str(str2num(listgnm(cont,:)));
  
  [ 'Current image set: ' true_value ]
  ...
  if imageExt == 'seg'
    [MS, nSegsMS] = berkeleyImLoadBySegments(pathMS);
  else
    [MS, nSegsMS, hist] = im2segs2(pathMS);  
  end;  
  
  [ 'Image labeled, ' int2str(nSegsMS) ' segments' ]
  ...
  for contEval = 1 : size(listseg,1)
      ...
      [ 'Comparing to ground truth ' int2str(contEval)  '/' int2str(size(listseg,1)) ]
